...
export function BillsList() {
  ...
  useEffect(() => { api.get("/bills").then((r) => setBills(r.data)).catch(() => {}); }, []);
  return (
    <div>
          ...
          {bills.map((b) => (
            <Link key={b.id} to={`/bills/${b.id}`} data-testid={`bill-card-${b.id}`}>
              <Card className="p-5 rounded-2xl hover:shadow-md transition-shadow cursor-pointer h-full">
                ...
                <div className="text-sm text-muted-foreground mb-3">{b.customer_phone || "—"} · {b.items?.length} items</div>
                <div className="text-3xl font-black text-accent">{inr(b.total)}</div>
                <div className="text-xs text-muted-foreground mt-2">{new Date(b.created_at).toLocaleString()}</div>
              </Card>
            </Link>
          ))}
        </div>}
...
export function BillDetail() {
  ...
  const load = () => api.get(`/bills/${id}`).then((r) => setBill(r.data)).catch(() => {});
  useEffect(() => { load(); api.get("/settings").then((r) => setSettings(r.data)); }, [id]);
  ...
  return (
    <div>
      <PageHead
        ...
        subtitle={`${bill.customer_name} · ${new Date(bill.created_at).toLocaleString()}`}
        action={
        ...
        <Card className="lg:col-span-2 p-8 rounded-2xl">
          <div className="flex items-start justify-between mb-6 pb-6 border-b border-border">
            <div>
              ...
              <div className="text-sm text-muted-foreground mt-1">{settings?.park_name || "Adventure Park"}</div>
              <div className="text-xs text-muted-foreground">{settings?.address}</div>
              {settings?.phone && <div className="text-xs text-muted-foreground">Ph: {settings.phone}</div>}
            </div>
          ...
          <Card className="p-5 rounded-2xl">
            ...
            {settings?.upi_qr_url && (
              <div className="mt-3">
                ...
                <img src={settings.upi_qr_url} alt="UPI QR" className="w-full rounded-xl border border-border" />
                {settings.upi_id && <div className="text-center mt-2 text-sm font-bold">UPI: {settings.upi_id}</div>}
              </div>
            )}
            {!settings?.upi_qr_url && !bill.razorpay_link && (
